...
class LinkedInScraper {
  constructor() {
    ...
    this.datasetId = 'gd_lyy3tktm25m4avu764'; // LinkedIn
    this.baseUrl = 'https://api.brightdata.com/datasets/v3';
  }
  
  /**
   * Trigger scraping for multiple URLs
   * @param {Array} urls - Array of LinkedIn URLs
   * @returns {Promise<string>} Snapshot ID
   */
  async triggerScrape(urls) {
    logger.info('Triggering LinkedIn scrape', { urlCount: urls.length });
    ...
    try {
      ...
      if (error.response) {
        logger.error('LinkedIn scraper API error', {
