...
class TikTokScraper {
  constructor() {
    ...
    this.datasetId = 'gd_lu702nij2f790tmv9h'; // TikTok
    this.baseUrl = 'https://api.brightdata.com/datasets/v3';
  }
  
  /**
   * Trigger keyword-based discovery (TikTok-specific)
   * @param {string} keyword - Search keyword
   * @param {Object} options - Search options
   * @returns {Promise<string>} Snapshot ID
   */
  async triggerKeywordSearch(keyword, options = {}) {
    logger.info('Triggering TikTok keyword search', { keyword, options });
    
    const {
      num_of_posts = 100
    } = options;
    
    // Prepare payload - only include fields TikTok actually accepts
    const payload = [{
    ...
    if (num_of_posts) {
      payload[0].num_of_posts = num_of_posts;
    }
    ...
    logger.info('TikTok API Request', { url, payload });
    ...
    try {
      ...
      if (error.response) {
        logger.error('TikTok keyword search API error', {
   ...
   * @param {Array} urls - Array of TikTok URLs
   * @returns {Promise<string>} Snapshot ID
   */
  async triggerScrape(urls) {
    logger.info('Triggering TikTok scrape', { urlCount: urls.length });
    
    if (urls.length === 0) {
      throw new Error('No valid URLs to scrape');
    }
    
    // Prepare payload - TikTok requires country field
    const payload = urls.map(url => ({ url, country: '' }));
    ...
    try {
      ...
      if (error.response) {
        logger.error('TikTok scraper API error', {
          status: error.response.status,
          data: error.response.data
        });
      }
