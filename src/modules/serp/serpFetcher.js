...
class SerpFetcher {
   ...
   */
  async fetchResultsForPlatform(query, platform, googleDomain = config.serp.googleDomain, maxResults = config.serp.maxResults, onProgress = null) {
    ...
    for (const searchQuery of searchQueries) {
      try {
        ...
        const results = await this.fetchResults(searchQuery, googleDomain, maxResults, onProgress);
   ...
   * @returns {Promise<Array>} Array of search results
   */
  async fetchResults(query, googleDomain = config.serp.googleDomain, maxResults = config.serp.maxResults, onProgress = null) {
    ...
    for (let page = 0; page < maxPages; page++) {
      ...
      try {
        ...
        if (onProgress) {
          try { await onProgress(page + 1, maxPages); } catch (e) { /* ignore */ }
        }
